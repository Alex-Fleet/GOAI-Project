...
> 状态：**已实现**（一楼搭建完成，65 测试全绿；§9 D1–D7 定稿落地，见 §4.4 / §10）
...
### 4.4 检索起点：知识检索两路接入推理（L1 兜底）

**候选来源两路**：层1 定位候选 = 确定性图遍历（主）+ 知识检索（兜底，可选注入）。图遍历定位不到对象（引用不在工艺图上/对象类型不匹配）时，检索把事故对象映射回图上候选，随后照常走推理核把关。

- **两路索引**：入库时每块知识 chunk 向量化（bge-m3 → VectorStore 契约）并建 Jieba/BM25 倒排；两路索引的 payload 携带该 chunk 抽取出的图实体引用（label/id/name）——是「检索命中 → 图实体」的桥。
- **挂载点**：`TracingLoop(retriever=...)` 可选注入（`engine/agent/loop.py` 的 `Retriever` 协议）。L1 图遍历为空时调用，返回图上候选 `Candidate(节点, basis)`，进入正常「提议 → 把关」流程。
- **安全线**：检索只是起点，不是结论——候选 basis 必须真实存在于图上（二层从图反向回溯构造真实边），推理核沿图验证 + 属性判定把关，不绕过「依据必须真实」；候选与触发事实无关联（id/名称不匹配）不采信，宁缺毋滥、不硬出结论。
- **二层实现**：`KnowledgeRetriever`（BM25 关键词 + 向量两路 → 命中 chunk → payload 实体 → 图节点 + 反向工艺路径 basis），见 `scripts/serve.py` 演示（`entity_ref=P1` 直连、`DMC-50H` 走兜底、未知对象 FAILED）。
...
| 知识入库 | 知识文件（手册/规格/条款）→ 切块 → **LLM 辅助抽取**实体关系 → 人审 → 入 Ladybug | 入的是属性图，非向量库；离线建设工具，不参与运行时推理。检索索引（BM25 + 向量两路）同时产出，payload 带实体引用，是推理的检索起点（§4.4） |
...
> 实现状态：D1–D7 已全部落地（一楼 + 演示服务）。
> - **D1** `engine/platform/graph.py`（Ladybug 实现，spike 通过；backward 查询方向已修正——EdgeRef 还原真实图方向，见 `tests/test_graph.py`）；
> - **D3** `engine/frontend/`（React 18 + Vite 5 + @xyflow/react 双栏工作台：左聊天 SSE 流式、右推理路径图 + 处置确认）；
> - **D5** `engine/service/`（FastAPI + SSE，`/start` `/events` `/chains/{id}` `/confirm` `/stream/{id}`，CORS 已配）；
> - **D6** `engine/kb/`（切块 → LLM 抽取 → 人审 → 入图/索引两路全链路）+ 检索挂载点（§4.4）；
> - **演示** `scripts/serve.py`（mock 知识/业务 + 引擎，端口 8800）：`entity_ref=P1` 图遍历直连、`DMC-50H` 检索兜底、未知对象 FAILED 不硬出结论。
> - 验证：65 测试全绿（正常 / 异常 / 审计 / KB / 检索兜底 / 图查询），ruff check + format 干净，前端 `tsc -b && vite build` 通过。

## 10. 开发与验证策略

0. **选型 spike**：已完成——Ladybug 0.19.1 Python 绑定在当前 Mac 正常加载，建图 / Cypher 遍历 / 事务 / 持久化验证通过（未触发 Issue #681）；工程约束已固化到 `engine/platform/graph.py` 注释（文件路径 / 无显式事务 / 强 schema / 关系表缺失返回空）。高级特性不依赖。
